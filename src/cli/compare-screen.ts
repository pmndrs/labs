...
import { comparisonSummary, renderCompareView, type CompareFrame } from './compare-view.ts';
...
export async function showCompareReport(result: CompareResult, config: LabsConfig): Promise<void> {
  if (
    ...
    result.environmentFailures.length ||
...
/** Keep input and terminal ownership inside one lifetime, including failed redraws. */
export function openCompareScreen(
  ...
  return new Promise((resolve, reject) => {
    ...
    function draw() {
      if (!active) return;
      try {
        frame = renderCompareView(result, config, {
    ...
    function onKey(text: string, key: Key) {
      ...
      if (key.name === 'up') selected--;
        ...
        selected = 0;
        listOffset = 0;
      } else if (key.name === 'end') selected = result.benches.length - 1;
      else if (text === '[' || key.name === 'left') detailOffset--;
      else if (text === ']' || key.name === 'right') detailOffset++;
      else return;
      selected = Math.max(0, Math.min(result.benches.length - 1, selected));
